...
f = open('static/symptom.txt', 'r', errors='ignore')
m = open('static/pincodes.txt', 'r', errors='ignore')
...
GREETING_RESPONSES = ["hi, are you suffering from any health issues?(Y/N)", "hey, are you having any health issues?(Y/N)",
                      "hii there, are you having any health issues?(Y/N)", "hi there, are you having any health issues?(Y/N)",
                      "hello, are you having any health issues?(Y/N)", "I am glad! You are talking to me, are you having any health issues?(Y/N)"]
Basic_Q = ("yes", "y")
...
def response(user_response):
    ...
    if req_tfidf == 0:
        ...
        robo_response += "\nDo you want to know nearby hospitals? (Y/N)"
        return robo_response
...
def get_doctors_by_pincode(pincode):
    doctors = []
    with open('static/pincodes.txt', 'r') as file:
        lines = file.readlines()
        for line in lines:
            if line.startswith(pincode + "-->"):
                start_idx = len(pincode) + 3
                doctors.append(line[start_idx:].strip())
                continue
            elif doctors:
    ...
    if doctors:
        ...
        return "Sorry, no doctors found for the provided pincode."
...
def chat(user_response):
    ...
    if user_response != 'bye':
        if (user_response == 'thanks' or user_response == 'thank you'):
            ...
            elif (user_response.isdigit() and len(user_response) == 6):
                return get_doctors_by_pincode(user_response)
            else:
